    ...
    func bindViewModel() {
        ...
        Observable.combineLatest(outputs.smallPhotoURL, outputs.regularPhotoURL)
            .flatMap { smallPhotoURL, regularPhotoURL -> Observable<ImageResponse> in
                return Observable.concat(this.imagePipeline.rx.loadImage(with: smallPhotoURL).asObservable(), this.imagePipeline.rx.loadImage(with: regularPhotoURL).asObservable())
        }
        .orEmpty()
        .map { $0.image }
